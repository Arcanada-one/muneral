...
//   * The injected clock resets in beforeEach and IDs are random, so a test run
//     in isolation with `-t` sees the same temporal setup as a full run.
//   * The suite creates its own fixed owner/workspace/project parent fixtures;
//     it does not rely on execution_authority_preseed.sql having run.
...
describeIf('ExecutionAuthorityService — disposable PostgreSQL', () => {
  ...
  it('16: duplicate attempt UUID → UnexpectedUniqueViolationError, not a phantom stale version', async () => {
    ...
    // tasks. Current main allocates three IDs per issued attempt: attempt,
    // transition, then MUN-0021 outbox event.
    const collidingAttemptId = randomUUID();
    let call = 0;
    const collidingIdSource: IdSource = {
      generate: () => (call++ % 3 === 0 ? collidingAttemptId : randomUUID()),
    };
